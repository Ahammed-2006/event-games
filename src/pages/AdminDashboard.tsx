...
import { BASELINE, overrideScores, type StudentScore, type StudentStatus } from '../services/scoreService';
import {
...
export default function AdminDashboard() {
  ...
  // ── Event handlers ────────────────────────────────────────────────────────
  const handlePause  = () => { setEventStatus('paused');  setConfirmModal(null); showToast('⏸ Event paused — student timers frozen.'); };
  const handleResume = () => { setEventStatus('live');    setConfirmModal(null); showToast('▶ Event resumed — timers running.'); };
  const handleEnd    = () => { setEventStatus('ended');   setConfirmModal(null); showToast('🏁 Event ended — scores locked.'); };

  const handleReset = () => {
    const reset = BASELINE.map(s => ({ ...s, score: 0, wordSearch: 0, imagePuzzle: 0, jigsaw: 0, status: 'not-started' as StudentStatus, time: '—' }));
    overrideScores(reset);
    overrideData(reset);
    setEventStatus('live');
  ...
  const handleExport = () => {
    const rows = [
      ['#', 'Name', 'Team', 'Word Hunt', 'Image Puzzle', 'Jigsaw', 'Total', 'Status'],
