...
import { Search, Image as ImageIcon, Grid3X3, Terminal, Clock, Star, ArrowRight, LogOut, User, Trophy } from 'lucide-react';
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 font-mono text-sm flex items-center gap-2">
              <User className="w-3 h-3" /> {user?.teamName || 'No Team'}
            </p>
          </div>
        </div>
            ...
            >
              <TiltCard intensity={10}>
                <Link to={challenge.path} className="group block h-full">
                  <div
                    className={`glass-card p-6 h-full flex flex-col border border-white/10 ${challenge.borderHover} ${challenge.bgHover} transition-all duration-300 rounded-xl`}
                    style={{
                      boxShadow: `0 4px 30px rgba(0,0,0,0.2)`,
                    }}
                    onMouseEnter={e => {
                      (e.currentTarget as HTMLDivElement).style.boxShadow = `0 8px 40px ${challenge.glowColor}`;
                    }}
                    onMouseLeave={e => {
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-5">
                      <div className={`p-3 rounded-xl bg-dark-bg/60 border border-white/5 ${challenge.color}`}>
                    ...
                    {/* Title + desc */}
                    <h2 className={`text-xl font-bold mb-2 font-mono ${challenge.color}`}>{challenge.title}</h2>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between text-xs text-gray-500 font-mono">
                        ...
                        <div className={`font-bold ${challenge.color}`}>+{challenge.points} pts</div>
                      </div>
                      <div className="pt-3 border-t border-white/10 flex items-center justify-between">
                        <span className={`font-bold text-sm tracking-wider ${challenge.color}`}>PLAY NOW</span>
                        <ArrowRight className={`w-4 h-4 ${challenge.color} group-hover:translate-x-2 transition-transform duration-200`} />
                      </div>
                    </div>
                  </div>
                </Link>
