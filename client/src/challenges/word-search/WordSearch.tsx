...
export default function WordSearch() {
  ...
  useEffect(() => {
    const handlePointerMove = (e: PointerEvent) => {
      ...
      if (!target || !target.dataset.row || !target.dataset.col) return;
              ...
              {gameState.grid.map((row, r) => (
                row.map((char, c) => (
                  <div
                    key={`${r}-${c}`}
                    className={cn(
                      "w-6 h-6 sm:w-10 sm:h-10 flex items-center justify-center text-sm sm:text-lg font-mono font-bold rounded cursor-pointer transition-colors select-none",
                      isCellSelected(r, c)
